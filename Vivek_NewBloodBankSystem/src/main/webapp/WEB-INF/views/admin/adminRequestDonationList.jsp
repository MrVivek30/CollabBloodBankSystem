...
<body>
    ...
    <th>Given Remark</th>
            ...
            <c:forEach var="request" items="${requestedDonationList}">
            <tr>
    <td>${request.id}</td>
    <td>${request.name}</td>
    <td>${request.age}</td>
    ...
    <td>${request.type}</td>
                               ...
                               <c:choose>
                                 <c:when test="${request.getStatus() eq 'Pending'}">
                                     <td>
                                         <a href="donateAccept/${request.uniqueId}">Accept</a><br/><br/>
                                         <a href="donateReject/${request.uniqueId}">Reject</a>
                                     </td>
                                 </c:when>
                                 <c:otherwise>
    <td>N A</td>
    ...
    ${request.remark}
                              ...
                              <c:choose>
                               <c:when test="${request.getStatus() eq 'Rejected'}">
                                  <td>
                       <form action="/com/BloodBank/v1/admin/donateRemark" method="post" name="${request.uniqueId}" id="${request.uniqueId}">
                <input type="hidden" id="userId" name="uniqueId" value="${request.uniqueId}">
                           <label for="remark">Remark:</label>
                           <textarea id="remark" name="donateRemark" rows="4" cols="5"></textarea>
